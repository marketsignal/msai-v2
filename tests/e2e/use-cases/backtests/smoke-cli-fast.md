...
- After step 3, `--json` stdout is a single parseable JSON document. Top-level keys are `id`, `status`, `smoke`, `structural_problems`; the risk metrics are nested under `.metrics` (`jq '.status'` → `"completed"`; `jq '.structural_problems'` → `[]`; `jq '.metrics.total_return'` → a number; **`jq '.metrics.trade_count_by_strategy["__smoke__/smoke_market_order/AAPL"]'` → ≥ 1 AND `...["__smoke__/smoke_market_order/SPY"]'` → ≥ 1** (the per-instrument deterministic floor — EACH symbol's `smoke_market_order` must fire, so a single symbol's zero can never be masked by the other's volume); `jq -r '.metrics.benchmark_symbol'` → `"SPY"`; `jq -r '.metrics.smoke_config'` → `"fast"`; `jq '.metrics.alpha'` and `jq '.metrics.beta'` are non-null). No human-readable framing precedes or follows the JSON.
- After step 4, the `--config nightly` invocation's stdout prints the same metrics block shape but with `.metrics.smoke_config == "nightly"` and a wider date range; `.id` is distinct from step 1's id.

**Persistence:** From the same shell (or a fresh SSH session), Pablo re-invokes `cd backend && uv run msai backtest smoke --json` and pipes through `jq -r '.id'` to capture the new run id. He then confirms the run is durably recorded through two reads:

1. `curl -sf -H "X-API-Key: $MSAI_API_KEY" "http://localhost:8800/api/v1/backtests/history?smoke_only=true" | jq '.items[] | {id, smoke}'` — the runs from steps 1, 3, and 4 all appear with `smoke=true`. (The history row is a trimmed list projection — it carries the `smoke` flag but NOT the full metrics; `smoke_config` lives on the run-detail endpoint below, not here.)
2. `curl -sf -H "X-API-Key: $MSAI_API_KEY" "http://localhost:8800/api/v1/portfolios/runs/<captured-id>" | jq '{smoke_config: .metrics.smoke_config, benchmark_symbol: .metrics.benchmark_symbol, alpha: .metrics.alpha, beta: .metrics.beta}'` — the step-1 id re-reads `smoke_config: "fast"` and step-4's re-reads `smoke_config: "nightly"`, with non-null `alpha`/`beta` vs the `SPY` benchmark.

The smoke runs persist across shell sessions and across a stack restart (`docker compose -f docker-compose.prod.yml restart backend`) — re-running read #1 after the restart still lists them with `smoke=true`.
...
- ANY `smoke_market_order/<symbol>` produces 0 trades (the per-instrument deterministic floor — each of AAPL and SPY is guaranteed exactly one `smoke_market_order` fill) → `structural_problems` names the offending strategy (e.g. `__smoke__/smoke_market_order/AAPL produced 0 trades; ...`) and the process exits non-zero. This is the structural FAIL that catches order-submission / instrument-resolution / catalog-staleness breakage for an INDIVIDUAL symbol (a per-symbol zero can no longer hide behind another symbol's volume — the regression guard for the 2026-05-26 AAPL=0/SPY=440 prod incident).
