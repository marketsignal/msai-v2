...
- After step 3, `--json` stdout is a single parseable JSON document. Top-level keys are `id`, `status`, `smoke`, `structural_problems`; the risk metrics are nested under `.metrics` (`jq '.status'` → `"completed"`; `jq '.structural_problems'` → `[]`; `jq '.metrics.total_return'` → a number; `jq '.metrics.trade_count_total'` → ≥ 2; `jq -r '.metrics.benchmark_symbol'` → `"SPY"`; `jq -r '.metrics.smoke_config'` → `"fast"`; `jq '.metrics.alpha'` and `jq '.metrics.beta'` are non-null). No human-readable framing precedes or follows the JSON.
