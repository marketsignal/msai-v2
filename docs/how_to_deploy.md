...
Azure VM (Ubuntu 24.04 LTS, Standard_D4ds_v6)
  ...
  /var/lib/msai  ← Premium SSD data disk (Parquet, postgres, redis AOF)
```

**Slice 4** (PR #58) layered on: nightly `pg_dump` → Blob backups, alert rules, and the active-`live_deployments` deploy gate (refuses to deploy if a broker is trading — operator clears state with `msai live stop --all` first).
...
- Changed `Dockerfile.dev`, `requirements.txt`, `pyproject.toml`, or `package.json`
...
The deploy script (`scripts/deploy-on-vm.sh`) auto-rolls-back to the previous SHA on probe failure. Manual rollback is for "the new code is fine but we want to revert behavior."
...
```bash
msai live stop --all          # graceful: cancels orders, flattens positions
# OR
msai live kill-all            # emergency: 4-layer halt if backend is wedged
```
...
```bash
# Provision a rehearsal RG (re-runs Bicep with a different RG name)
RG=msaiv2-rehearsal-$(date +%Y%m%d)
./scripts/deploy-azure.sh --resource-group "$RG"

# Get the rehearsal VM's IP + host key
VM_IP=$(az deployment group show -g "$RG" -n main --query 'properties.outputs.vmPublicIp.value' -o tsv)
ssh-keyscan -t ed25519 "$VM_IP"   # save this multi-line output

# Dispatch deploy with rehearsal overrides
gh workflow run deploy.yml \
  -f resource_group="$RG" \
  -f vm_public_ip="$VM_IP" \
  -f nsg_name="msai-vm-nsg" \
  -f kv_name="$(az keyvault list -g "$RG" --query '[0].name' -o tsv)" \
  -f msai_hostname="platform-rehearsal.marketsignal.ai" \
  -f acr_name="$(az acr list -g "$RG" --query '[0].name' -o tsv)" \
  -f acr_login_server="$(az acr list -g "$RG" --query '[0].loginServer' -o tsv)" \
  -f vm_ssh_known_hosts_var="<ssh-keyscan output>" \
  -f bootstrap=true

# After success, tear down
az group delete -n "$RG" --yes --no-wait
```

Full rehearsal procedure: [`docs/runbooks/slice-3-first-deploy.md`](runbooks/slice-3-first-deploy.md) §Rehearsal-RG smoke.
...
CI needs ~15 repo Variables and a handful of Secrets. **Variables** are non-sensitive (visible to anyone with read on the repo); **Secrets** are encrypted (only deploy-time access). Set via `gh variable set NAME --body 'value'` and `gh secret set NAME`.
...
| `NSG_NAME`                    | `msai-vm-nsg`                      | transient SSH rule |
...
| `DEPLOYMENT_NAME`             | `prod`                             | log tagging        |
...
The Slice 1 acceptance step (in [`docs/runbooks/vm-setup.md`](runbooks/vm-setup.md)) walks through populating these from Bicep outputs.
...
| `deploy.yml` fails at "Refuse if active live_deployments" | Active broker subprocess; or backend unreachable  | `msai live stop --all`; or check `gh run view --log` for FAIL marker                                   |
...
| Orphan transient NSG rule                                 | Cleanup job failed                                | `reap-orphan-nsg-rules.yml` cron picks up within 15 min                                                |
...
- **VM-side scripts** (staged on every deploy under `/opt/msai/scripts/`):
  - `scripts/deploy-on-vm.sh` — pulls images, renders env, runs `docker compose up -d --wait`, rolls back on failure
