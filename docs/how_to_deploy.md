...
| 8          | `gh workflow run deploy.yml -f bootstrap=true -f git_sha=<sha7-from-row-5>` — `git_sha` is REQUIRED after the local docker buildx path because deploy.yml's workflow_dispatch fallback resolves to `GITHUB_SHA::7` (i.e., whatever's currently on main), which is NOT the rehearsal SHA you just pushed. The `-f` override matrix on top of these two is only needed if you choose NOT to swap the repo Variables in step 7.                                                                                                                                                       | runbook §8                                                      |
