...
| 5          | Push images for the rehearsal SHA. `build-and-push.yml` is gated on `if: github.ref == 'refs/heads/main'`, so `gh workflow run` from a feature branch is rejected. Two options: (a) merge to main first then rehearse — defeats the "rehearse pre-merge" purpose; (b) build + push locally via `docker buildx build --push -t <rehearsal-acr>/msai-{backend,frontend}:<sha7> ...` against the rehearsal ACR. The runbook's terse `gh workflow run build-and-push.yml` step assumes the latter path; spell it out in your actual run. | runbook §5 (gap)                                    |
