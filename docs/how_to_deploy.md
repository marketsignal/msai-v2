...
| 4          | Seed rehearsal KV with secrets (report signing, postgres password, Entra IDs, CORS, IB stubs, plus `MSAI_API_KEY` for the active-deployments gate's curl auth and `DATABENTO_API_KEY` for the bootstrap/ingest path — runbook §4 only lists the original set; add these two before dispatch)                                                                                                                                                                                                                             | runbook §4 (partial — missing MSAI_API_KEY + DATABENTO_API_KEY) |
| 5          | Push images for the rehearsal SHA. The runbook literally writes `gh workflow run build-and-push.yml`, but `build-and-push.yml`'s jobs are guarded `if: github.ref == 'refs/heads/main'` — a feature-branch dispatch produces zero ran jobs. Use one of: (a) merge to main first then rehearse — defeats the pre-merge purpose; (b) build + push locally via `docker buildx build --push -t <rehearsal-acr>/msai-{backend,frontend}:<sha7> ...` against the rehearsal ACR. Do NOT follow the runbook's literal step here. | runbook §5 (broken — follow this row instead)                   |
