...
## Addendum 2026-06-03 — PR 1b (data-stale auto-halt) implemented

**Blocking objection #9 is satisfied** by `feat/pr1b-data-stale-auto-halt`. Shape: **in-node** observation + evaluation (Contrarian-VALIDATED 2026-06-03 — "node-side observation/evaluation is the only freshness source and keeps the real-money halt path supervisor-independent"; the Nautilus 1.223.0 Databento adapter exposes no connection-state/last-event hooks, and `is_connected` stays True through silent Rust-side reconnects, so event-arrival observation in the node is the only viable signal). `DataFreshnessActor` records `bar.ts_event` per `FeedKey(dataset, native_bar_type)`; `DataStaleMonitor` (the IBDisconnectHandler injected-loop shape) applies session-aware, phase-open-clamped, grace-only-config budgets at two granularities and on stale sets the SAME fleet emergency latch with `reason=data_stale` + source/`detected_at`/`last_event_ts` cause via an atomic `HALT_WRITE_LUA` (SET-NX cause + capped history — a manual `/kill-all` no longer erases a data-stale attribution, and the pre-existing `IBDisconnectHandler` ad-hoc `:reason`/`:source` divergence is converged on `halt_cause_key()`, new `HaltCause.IB_DISCONNECT`). `/resume` is now fail-closed (manifest-present + every-feed-warm + node-published reconciled marker, cleared atomically by `RESUME_CLEAR_LUA` — the warms-then-re-stales race is closed exactly, per the PRD edge case). Operator surfaces: `GET /api/v1/live/data-health` + `msai live data-health`; Redis-hydrated labeled metrics on the bare `/metrics` scrape. The five objection-#9 failure modes (disconnect, stale-timestamp, partial-dataset stall, single-symbol stall, reconnect-storm) are proven by a 25-test simulated harness with correct per-mode attribution + no-false-halt negatives (closing gaps, closed/maintenance sessions, session reopen, thin 5-minute feeds). **Scope honesty:** the live topology is equities-only today — GLBX/futures freshness ships as config + CMES sessions + simulated coverage, activating when futures nodes ship; OPRA deferred to the capacity spike. The LVP/HVP real-money graduation drill (market-hours) remains the operator's closing gate, now unblocked.
