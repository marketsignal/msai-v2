...
- **Decision core (`backend/src/msai/services/gateway_watchdog.py`):** pure, unit-tested FSM `decide(Signals) -> Decision`. Health enum {HEALTHY, UNHEALTHY, STARTING, UNKNOWN}; actions {NONE, RESTART, ALERT_ONLY, ESCALATE, RECOVERED}. Confirmed-healthy (running AND healthy AND connected) is the ONLY counter-reset path; `STARTING` (respects the gateway's 180s start_period) and ambiguous signals preserve state. Grace window before acting; **live-deployment-active → ALERT_ONLY (never restart out from under a node — gotcha #13)**; anti-flap restart cap K=3 in 15 min → ESCALATE + CRITICAL + 30 min cooldown → ONE post-cooldown retry → re-escalate. All thresholds env-tunable. 16 unit tests.
- **Host signal/act seam (`backend/src/msai/cli.py`):** `system gateway-watchdog-tick` (host passes `docker inspect` container running/health as bare Typer flags; CLI adds the in-app signals — `/account/health` body-parsed for `gateway_connected`/`status` [endpoint returns 200 even when down], and the **DB-direct** active-deployment count over `LiveDeployment.status in ACTIVE_DEPLOYMENT_STATUSES`, conservative-on-error), runs `decide()`, persists `WatchdogState` to Redis, alerts via `AlertService`, prints `decision=… reason=… restart_count=…` + a trailing action token. `system gateway-watchdog-reset` clears state. `--report-host-failure TEXT` raises a throttled (fail-open) CRITICAL when the host's `docker compose --force-recreate` or env-render fails. 5 integration tests (real Postgres testcontainer + fake Redis).
- **Host wiring:** `scripts/gateway-watchdog.sh` (flock non-overlap; missing-env → report-host-failure + exit 0; rc/empty-token guard before the restart block; `restart` token → `COMPOSE_PROFILES=broker docker compose --project-name msai --env-file /run/msai.env --env-file /run/msai-images.env -f docker-compose.prod.yml up -d --force-recreate ib-gateway` with rc capture). `msai-gateway-watchdog.service` (Type=oneshot) + `.timer` (OnBootSec=90, OnUnitInactiveSec=30s). Installed/enabled by `deploy-on-vm.sh` **after** the smoke phase + `trap - EXIT` (a smoke rollback to an image lacking the new CLI never leaves a stale failing timer); FATAL-on-enable-failure, no app rollback. `deploy.yml` stages the new scripts/units to the VM.
- **Non-goals:** does NOT fix auth/config failures (e.g. the HVP paper-vs-live login) — it escalates them via the CRITICAL path and stops storming; prod only (no dev); never spawns a duplicate gateway (respects IB one-session-per-login, gotcha #3).
- **Review:** 11-iteration plan loop (Codex) + 3-iteration code loop (Codex + pr-toolkit, both clean same head). E2E PASS (CLI + API surfaces; UI N/A) — `tests/e2e/reports/2026-06-10-17-44-ib-gateway-watchdog.md`: restart_count advanced 1→2→3 across separate ticks; 4th tick `decision=ESCALATE` surfaced a CRITICAL "persistent failure — operator needed" alert on `GET /api/v1/alerts/`, durable across re-request. Full suite green; ruff + mypy --strict + shellcheck clean. Runbook: `docs/runbooks/ib-gateway.md`.

### 2026-06-09 — Feature: dashboard account selector + real-money deploy identity gate (PR4, `feat/dashboard-account-selector`)

**Status:** fleet-management front-end for the multi-account IB broker fleet, paired with a server-side safety gate that makes a wrong-account real-money deploy impossible — directly addressing the wrong-account failure class hit in the 2026-06-08 HVP drill. API-first, enforced identically across UI/API/CLI.

- **Explicit account class (never inferred from the id string).** `BrokerAccount` gains an additive `account_class` (`paper`|`test`|`real`, `String(16)` StrEnum + `server_default='test'`, matching the `status`/`trading_mode` convention — migration `b50fd33a0a8c`, backfill `paper`-mode→`'paper'` / `live`-mode→`'test'`) + a derived `is_real_money` property. Surfaced in `BrokerAccountResponse`; accepted on create (API + `msai broker add --account-class`) with a class/mode matrix enforced at BOTH the request schema AND the service boundary (paper⟺paper, live⟺test|real).
- **Server-side real-money identity gate** on `POST /api/v1/live/start-portfolio`, inserted after `_resolve_effective_account` (on the RESOLVED account, never the raw request — halt-bypass-safe) and before the idempotency reservation: a real-money (`is_real_money`) target requires a `confirm_account_id` equal to the resolved `ib_account_id` → else `422 REAL_MONEY_CONFIRM_REQUIRED`/`REAL_MONEY_CONFIRM_MISMATCH`; a `broker_account_id` plus a conflicting legacy `account_id`/`ib_login_key` → `422 AMBIGUOUS_DEPLOY_TARGET`. One handler ⇒ enforced for UI/API/CLI. `--confirm-account-id` added to `msai live start-portfolio` + `live start`; a non-gating `DEPLOY_TARGET_DIVERGENCE` metric records UI selector-context vs. resolved-target divergence.
- **Global account selector (view-scope only).** A top-bar `Select` (React Context + `usehooks-ts` localStorage, default "All", hydration-safe) re-scopes the live-trading fleet view + dashboard cards to one account, persists across navigation + reload, leaves backtesting untouched. Union population (registered ∪ deployment-seen ∪ All ∪ Unassigned ∪ Unknown/retired), real-money accounts unmistakably labeled. Deploy dialog rewritten to a registry-backed EXPLICIT target picker (no more free-text/id-prefix parsing); "All"/"Unassigned" disable Deploy; real-money targets require a typed account-id echo. Settings broker UI + `/account` + dashboard balance labels key off `is_real_money`/the `account_id` now returned ATOMICALLY in the `/account/summary` payload (gateway-bound, honestly labeled). Account-scoped views fetch BOTH default `/live/status` (table: all-status) and `active_only=true` (complete active set for scoping/counts) so the 50-row cap never drops an active deployment's positions.
- **Quality gates:** plan-review loop CERTIFIED at 9 Codex iterations; code-review loop CERTIFIED (Codex `exec review` + PR-toolkit BOTH clean, no P0/P1/P2 — all findings P2-only across the run). Full backend suite **3639 passed / 0 failed**; ruff + mypy --strict clean (222 files); frontend tsc + eslint + build clean. E2E verify-e2e PASS (safe subset: UI selector/dialog + API/CLI 422-rejections, live vs a worktree stack) — report in `tests/e2e/reports/2026-06-09-pr4-dashboard-account-selector-safe-subset.md`.
- **Attended follow-up (council gate exception, non-blocking):** the matching-confirm real-money DEPLOY happy-path is integration-covered (`test_real_money_deploy_confirm_match_passes_gate`) and reserved for an attended drill (real-money safety rails); not run unattended.
- **Council:** PR4 product/UX design (5 advisors + Codex chairman, 2026-06-08); E2E-execution decision (5 advisors + chairman, 2026-06-09 — safe-subset verdict).
...
- **In-node freshness observation** — `DataFreshnessActor` (wired via `ImportableActorConfig` next to the symbology shim) records `bar.ts_event` + arrival per `FeedKey(dataset, native_bar_type)` into a thread-safe registry (EVENT time drives staleness, so a feed delivering bars frozen in the past goes stale even though arrivals continue; full bar-type identity means 1-min and 5-min subscriptions are distinct feeds with their TRUE parsed cadence).
- **`DataStaleMonitor`** (the `IBDisconnectHandler` injected-loop shape) evaluates session-aware two-granularity grace (per-feed fresh / per-dataset alive) with a **phase-open clamp** (no reopen false-halts; budget restarts at phase open) against a **grace-only config** (`DATA_FRESHNESS_GRACE_JSON`, per asset-class × session phase; equities pre/post anchored to the calendar's REAL session bounds so early-close days shift correctly; GLBX maintenance window never-stale). On stale: sets the existing fleet emergency latch + canonical `halt_cause_key()` cause (`reason=data_stale` + source account/node/dataset/feed/symbol + `detected_at` + `last_event_ts`) via a new **atomic `HALT_WRITE_LUA`** (cause SET-NX preserves an existing cause; every cause appends to a capped `:history`). The PR 2/F6 node order gate blocks new opening orders fleet-wide ≤2s with ZERO gate changes. Publishes per-feed freshness JSON + plain `:verdict` companions + a TTL'd required-feed **manifest** every tick (a SIGKILLed node's manifest self-expires; legacy/non-Databento nodes publish an EMPTY manifest = "no feeds required").
- **Fail-closed `/resume`** — before clearing, verifies per ACTIVE deployment (the canonical 5-status set incl. `stopping`): manifest present (absent → 409 `RESUME_BLOCKED_DATA_STALE`), every manifest feed's verdict warm (missing row → blocked), reconciliation marker present (→ 409 `RESUME_BLOCKED_RECONCILIATION`); the clear itself is the **atomic `RESUME_CLEAR_LUA`** (re-verifies manifests still exist + every verdict warm + reconciled, THEN deletes latch + all cause keys incl. the legacy `:reason`/`:source` residue — closing the warms-then-re-stales race exactly). Reconciled marker = node-published Redis key set only after a healthy reconcile (nautilus 1.223.0: a failed reconcile never reaches `trader.start()`, so readiness implies reconcile-true), DELETEd at subprocess start.
- **Cause-key convergence (pre-existing bug fixed):** `IBDisconnectHandler` now writes the canonical `halt_cause_key()` via the same Lua (new `HaltCause.IB_DISCONNECT`) instead of ad-hoc `:reason`/`:source` keys that `/resume` never cleared; `/kill-all` switches its halt write to the same Lua so a manual kill-all **preserves** an existing data-stale attribution (history records both) instead of silently erasing it.
- **Operator surfaces** — `GET /api/v1/live/data-health` + `msai live data-health`: per-feed warmth (account/node/dataset/feed/symbol, `last_event_ts`, phase, grace, verdict incl. API-derived `missing`) + fleet-halt banner distinguishing `data_stale` from a manual cause. Metrics are **Redis-hydrated** on BOTH the data-health route and the bare `/metrics` scrape (`DATA_FEED_AGE_SECONDS`/`DATA_FEED_STALE` labeled `{account,node,dataset,symbol,feed}`, `DATABENTO_DATASET_ALIVE`, `IB_EXEC_PACING_ERRORS` from the node's OrderRejected pacing-phrase counter); gauge children are REPLACED per hydration so removed feeds disappear.
...
- **Migration `81e7efe6d772`** — nullable, additive `live_deployments.broker_account_id` FK → `broker_accounts.id` (`ondelete=RESTRICT`; broker accounts are soft-archived, never hard-deleted) + nullable `credentials_validated_at` / `credentials_validated_version` stamps. Legacy/default deployment rows survive untouched (no backfill; rollback-safe).
- **`PortfolioStartRequest`** — optional `broker_account_id` selector + an **either/or** validator (`broker_account_id` OR both legacy `account_id`+`ib_login_key`); `LiveDeploymentInfo` exposes `broker_account_id` so the linkage is observable through the API.
- **`deployment_account_resolver.py`** — `resolve_active_broker_account` (by id or unique-active `ib_account_id`, else fail closed) + a **two-stage** validation gate: `validate_account_row_state` (cheap, no KV: archived / mode-consistency / route-existence via `GatewayRouter.resolve()` / binding via `accounts_for()`) and `validate_account_credentials` (the credential read). The credential-resolvability check covers the backends the **backend API process can actually read** — `azure_kv` (managed identity reads Key Vault) and `env` (the dev file-backed store is mounted) — confirming their credentials **resolve** and are **version-pinned**. **`legacy_env` accounts (the migrated LVP/HVP backfill whose `credentials_secret_ref` points at gateway env keys, e.g. `env:TWS_USERID|TWS_PASSWORD`) validate ROW-STATE ONLY**: those `TWS_*` env vars live only in the IB Gateway containers, never the backend, so the deploy gate **skips** the (unreadable) env read for `legacy_env` and **defers credential authority to the IB Gateway**, which authenticates with them at spawn. `version=None` for `legacy_env` (NULL secret version → STAGE-3 rotation compare is `None==None`, no false trip). Without this skip a selector deploy of a backfilled LVP/HVP account would have fail-closed with a spurious credential-not-found 422 in the backend, making LVP/HVP undeployable. Archived / mode-inconsistent / route-unresolvable / not-router-bound are checked by `validate_account_row_state` (STAGE 1) + the STAGE-3 locked re-check for ALL backends including `legacy_env`, so skipping the credential read opens no fail-closed gap. The gate proves account identity + credential **resolvability** (for the readable backends) — it deliberately does NOT compare `tws_userid` to `ib_login_key` (the latter is a GatewayRouter routing alias, not the TWS username, so the comparison would falsely reject migrated/aliased accounts), and it does NOT prove the live gateway authenticated with those credentials.
- **`/api/v1/live/start-portfolio`** — establishes an **effective account** (derived `account_id`/`ib_login_key`) used by EVERY downstream safety/identity check (identity signature, idempotency body-hash, `(revision_id, account_id)` collision, the per-account **halt gate**, terminal-halt). Cheap row-state validation runs before idempotency hashing; the side-effectful KV validation runs AFTER `idem.reserve()` and BEFORE the `starting` upsert (a replay never re-pokes KV; an invalid-cred request never persists a row). New free-form deploys must resolve to one active account or fail closed (council mandate); warm-restart of existing deployments keeps the legacy string path. `DEPLOY_VALIDATION_FAILED` metric for row-state failures (cred failures already counted by `SPAWN_FAILED`).
- **CLI** — `msai live start-portfolio` and `msai live start` gain `--broker-account-id`; client-side either/or guard relaxation; thin HTTP client (surfaces the API's fail-closed error to stderr + non-zero exit).
- **Honesty (council minority report):** the gate proves account identity + credential **resolvability** — it does NOT prove the live gateway authenticated with those credentials (that is the deferred Option B / gateway-env render). Stated in the resolver docstrings + this entry. A regression test asserts the supervisor per-spawn/warm-restart path makes **no** KV call (the credential resolution stays on the control-plane deploy path).
- **Deferred (tracked):** Option B (wire credentials into the gateway-env render) + Option C (per-account gateway containers, bound to the 2-VM split); the LVP/HVP real-login graduation drill (market-hours).
...
- **`broker_accounts` table + entity** (`models/broker_account.py`): UUID PK, `ib_account_id` (immutable), `ib_login_key`, `status` (active/archived), `gateway_slot`, `trading_mode`, `credentials_backend` (azure_kv/env/legacy_env), `credentials_secret_ref` + pinned `credentials_secret_version` + audit columns (`credentials_updated_at/by`, `credentials_last_accessed`). Two partial-unique indexes (`ib_account_id`, `gateway_slot` WHERE not archived). Migration `d87c2aa5f751`.
- **Option B' credential storage** — dedicated, writable `BrokerCredentialsStore` (`services/live/broker_credentials_store.py`), separate from the read-only `SecretsProvider`: prod `AzureKvBrokerCredentialsStore` (explicit `ManagedIdentityCredential`, version-pinned `get`, set/begin_delete, payload-validation→`DECRYPT_FAILED`) + dev `EnvFileBrokerCredentialsStore` (0o600 file, per-ref versioning, fail-closed classification). `azure-keyvault-secrets`/`azure-identity` moved to base deps. Credentials NEVER returned by any read (response schema metadata-only) and masked in 422 validation echoes (global `RequestValidationError` handler, normalized credential-key heuristic).
- **`BrokerAccountService`** — create (store-write-first → bounded slot-allocation retry under per-attempt SAVEPOINT → single durable commit; orphan-secret best-effort cleanup), list/get, update (config + immutable-`ib_account_id` guard), rotate (legacy→managed promotion + half-rotation safety), archive (active-deployment-blocked, soft-delete + secret delete), `resolve_for_spawn` (the control-plane→data-plane seam for the supervisor; archived/null-version fail-closed; `SPAWN_FAILED` metric). Account-prefix↔mode guard (`DU`/`DF`=paper, `U`=live) shared by schema + service + migration via `assert_account_mode_consistent`.
- **API** `/api/v1/broker-accounts` (POST 201+Location, GET list [bare array]/{id}, PATCH, POST /{id}/rotate-credentials, POST /{id}/archive); **CLI** `msai broker` sub-app (add/list/show/rotate/archive; password via `$MSAI_BROKER_TWS_PASSWORD`/getpass, never argv); **UI** `/settings/broker-accounts` (list + detail [metadata only] + add wizard + Trust-First archive/rotate/edit dialogs).
- **Observability/IaC**: `broker_account_spawn_failed` counter + `kv_secret_age_seconds` gauge; boot KV-reachability probe (fail-closed in prod with active deployments); prod compose + `msai-render-env.service` + `.env.example` plumb `AZURE_KEYVAULT_URI`/`AZURE_KV_MI_CLIENT_ID`/`BROKER_GATEWAY_SLOTS`/`BROKER_ACCOUNT_BACKFILL`; `infra/main.bicep` adds a VM-MI **Key Vault Secrets Officer** assignment (write). `AZURE_KEYVAULT_URI` is **derived from `KV_NAME`** in `render-env-from-kv.sh` (not fetched as a circular KV secret), and `deploy-on-vm.sh` Phase 5 now re-syncs the render unit + script on every deploy (`deploy.yml` stages both) so render-logic changes reach existing VMs rather than being frozen at cloud-init provision time (code-review iter-15 prod deploy-break P1).
...
- **Lock-order invariant standardised (deployment-FIRST), all paths in one pass.** Global order is `advisory(gateway) → live_deployments FOR UPDATE → live_node_processes FOR UPDATE`; NO path holds a node-row lock while waiting on a deployment row. Migrated: `_clear_sentinel_and_refail_after_giveup`, `_refail_stranded_restart`, `FleetRouter.stop`, `live_stop` (api/live.py), `_mark_running` (trading_node_subprocess), and the bulk-UPDATE orderings in `watchdog_once`, `HeartbeatMonitor._mark_stale_as_failed`, and the rescan Step-1 flip. Each re-validates the owned node row by id after locking. Module-level per-path lock-order table + acyclic wait-for-graph proof added to `fleet_router.py` (only edges are advisory→deployment and deployment→node; no node→deployment edge ⇒ no cycle).
- **Pre-start stop-intent gate (closes F3's last window).** `spawn_with_outcome` re-reads the latest node row's `stop_requested_at` AFTER Phase A commits the `starting` row but IMMEDIATELY BEFORE `process.start()`; if set (or halt set, already re-checked), it terminalizes the reserved row and ACKs without spawning — no live node for a stopped account.
- **Dropped the naked operator-path cancel (F2 fix) + shutdown-cancellation safety.** Removed `cancel_restart_task` from the top of `FleetRouter.stop` (it orphaned a committed `starting` row mid-reservation). `spawn_with_outcome` now terminalizes the reserved row on `CancelledError` in the reserved→pre-start window (deployment→node lock order) before re-raising. Operator-stop correctness rests on the durable `stop_requested_at` intent + the serialized Phase-A OPERATOR_STOPPED re-check + the pre-start gate. `FleetRouter.stop`'s no-active-row branch now stamps the latest failed row under the deployment lock (restores the immediacy the cancel provided).
- **Ownerless-active-row backstop (`_reap_ownerless_active_rows`).** Host-AGNOSTIC reconciling sweep in `watchdog_once` reaps any active row (incl. `starting`/`stopping`) with `pid IS NULL`, no live cached handle, older than the grace window: terminal-no-respawn when `stop_requested_at` is set, recoverable (`HEARTBEAT_TIMEOUT`) otherwise. Guarantees no cancel/crash variant can permanently wedge a deployment slot. Emits `ownerless_active_row_reaped`.
- **Tests:** new `tests/integration/live_supervisor/test_pr2_lock_order_concurrency.py` (concurrent stop-vs-give-up deadlock probe + static deployment-first inspection guard + pre-start gate + cancellation-safety + ownerless backstop incl. stop-intent/host-agnostic + normal stop/respawn unchanged). Targeted suite green (115 + 12 + live-start/drain), ruff clean, mypy --strict clean.

#### 2026-06-01 (follow-up) — completeness pass over the three invariants (queued-START gap + multi-row AB-BA + full-span cancellation)

A COMPLETENESS PROOF over the three concurrency invariants closed the residual gaps the original verdict did not scope. See the THREE-INVARIANT COMPLETENESS PROOF block in `fleet_router.py`.

- **INVARIANT 1 — operator-stop gates EVERY node-spawn path (queued-START gap, Codex P1 live.py:1740).** The no-active-row branch of both `FleetRouter.stop` and `live_stop` previously stamped `stop_requested_at` ONLY on a `failed` latest row. For a START that was queued-but-unconsumed (deployment `starting`/`building`, NO node row yet — or a non-`failed` latest row from a prior lifecycle), nothing was recorded, so the queued START later reached Phase A (`restart_carry is None`, deployment not terminal) and SPAWNED a node after the operator was told "stopped". Fix: the pre-active no-active-row branch now marks the **deployment** operator-terminal (`status="stopped"`) under the `live_deployments FOR UPDATE` lock; Phase A added an OPERATOR-TERMINAL DEPLOYMENT GATE that ABORTS (`OPERATOR_STOPPED`) on EVERY path (initial / redelivered START + `restart_carry` respawn) when `deployment.status == "stopped"`. `stopped` is now strictly distinguished from `failed`: `stopped` is operator-terminal (never reset, never respawned), `failed` stays recoverable (restart_carry / transient-retry reset). This also fixed a latent bug where a `stopped` deployment with a recoverable latest row COULD be resurrected by `restart_carry`.
- **INVARIANT 2 — deterministic total order on every multi-row deployment lock (pr-toolkit P1, D↔D AB-BA).** The deployment-FIRST rule kills D→N→D cycles but not a D↔D (AB-BA) deadlock between two concurrent multi-row deployment-set lockers with overlapping sets. Added `ORDER BY LiveDeployment.id` to the HeartbeatMonitor stale sweep Step-1 and the rescan Step-1a deployment `FOR UPDATE`; added an ordered `SELECT ... ORDER BY id FOR UPDATE` PRE-LOCK before the watchdog Step-3 bulk deployment UPDATE; made the `/drain` terminal-sync loop iterate deployments in sorted-id order. All multi-row deployment-set acquisitions now share one ascending-id order ⇒ no AB-BA. (No path locks a SET of node rows — node writes are single-row by id or scoped to a pre-locked deployment set.)
- **INVARIANT 3 — cancellation never orphans a reserved row (Codex P2 fleet_router.py:764).** The original cancellation cleanup wrapped only `process.start()` + handle-install; the `await self._payload_factory(...)` (and the halt re-checks / account-halt lookups / pre-start gate) ran OUTSIDE it. A shutdown `cancel_restart_tasks()` cancel in the payload-factory await orphaned the reserved `starting` pid=NULL row (`CancelledError` is `BaseException`, not caught by the factory's `except Exception`). Fix: extracted the entire reserved→handle-install span into `FleetRouter._phase_b_spawn_guarded`, wrapped in one `try / except asyncio.CancelledError` with a `handle_installed` flag — a cancellation at ANY await before the handle is installed terminalizes the reserved row (deployment→node lock order) before re-raising.
- **Codex P2 — transient-START retry attribution.** Phase A's transient-retry exemption honored ANY historical `SPAWN_FAILED_TRANSIENT` latest row; a later `/start` that timed out (504) without a node row could let a stale redelivery bypass the stale-START drop and spawn after the caller gave up. The exemption is now bound to the CURRENT attempt — only honored when the terminal row's `started_at` is at/after `deployment.last_started_at`.
- **Tests:** +14 in `test_pr2_lock_order_concurrency.py` (queued-START no-node-row + redelivery + prior-stopped-row + restart_carry-must-not-resurrect-stopped + failed-still-restarts; static ORDER-BY-id guard + deterministic barrier AB-BA probe + falsification + heartbeat×rescan + rescan×watchdog real-locker probes; cancel-during-payload-factory; stale-vs-current transient redelivery) + 1 API test (`test_live_start_endpoints.py`: /stop of a still-starting deployment marks it stopped). Full targeted suite green, ruff clean, mypy --strict clean.

#### 2026-06-01 (live real-money E2E + two E2E-found fixes)

Operator-authorized real-money E2E on the live LVP (`U4705114`) + HVP (`U4715997`) accounts, Databento `EQUS.MINI` live feed, IB execution. Report: `tests/e2e/reports/2026-06-01-pr2-per-account-supervisors.md`.

- **PR-2 headline PROVEN LIVE.** Kill-A-node drill on real accounts: SIGKILL'd LVP's TradingNode → **HVP node completely unaffected** (same pid, fresh heartbeats — no fleet-wide blast radius) → LVP auto-restarted in ~30s, re-reconciled the real account, `consecutive_respawn_failures` 1→0 after healthy reconcile. UC-API-1 (per-account health), UC-API-2 (drain isolation + durable latch), UC-CLI-1 (`msai live status` router-health line), UC-UI-1 (`/live-trading` supervisor-health column) all PASS. Node-side halt gate observed gating every live submit (`[Risk] SubmitOrder`); `MARKET_EXIT` reduce-only flatten passed the gate.
- **FIX — `symbology_shim.py` dead dataset (root cause of "no live data").** Equities mapped to `DBEQ.BASIC`, deprecated by Databento 2025-01-13; the live feed handshook but delivered zero instrument definitions + zero bars. Fixed → **`EQUS.MINI`** (the active live US-Equities-Mini dataset; confirmed via direct SDK test delivering trades + 1-min bars + symbol mappings). Swept `DBEQ.BASIC → EQUS.MINI` across tests + source comments (kept the deprecation note). After the fix, two cycling test strategies (`cycle_buy_sell`, `cycle_buy_sell_b`) traded SPY+AAPL on LVP and QQQ+MSFT on HVP, BUY/SELL every 2 min for ~12 min with real IB fills off the live feed — full path proven: `EQUS.MINI bar → on_bar → halt-gated submit → IB fill`.
- **FIX — `live.py` `MissingGreenlet` 500 on `/start-portfolio` poll-timeout.** The poll-timeout handler accessed `deployment.id` after `db.rollback()` expired the ORM object. Fixed → capture the PK as a plain local before the rollback. Verified live: `/start-portfolio` now returns a clean `503` with diagnostics instead of `500`.
- **Docs/config:** `EQUS.MINI` + the LVP-local/HVP-prod live-test model + real-fund-untouched-until-post-PR-3 documented in `CLAUDE.md`. Documented that read-only IB mode is incompatible with Nautilus startup reconciliation (code 321 → node never reaches `is_running`) — live nodes require orders enabled. (Polygon data-source rip-out tracked as a separate follow-up PR.)

#### 2026-06-01 (post-E2E code-review hardening — iters 25-28, operator-stop race-class closure)

Final dual-engine code-review iterations (Codex + pr-review-toolkit) over the real-money path closed the remaining operator-stop-vs-startup race surface. All FIXED + regression-tested before ship; final Codex RE-confirm deferred to the GitHub `chatgpt-codex-connector` bot per operator directive.

- **iter-25:** `data/ib-gateway-*/` broker runtime state/secrets added to `.gitignore` (never committed); cycle test strategies advance `_is_long` in `on_order_filled` (not pre-submit) + instrument-guard so a halt-blocked BUY never desyncs into selling an external position; graduation pipeline lints the halt-gate supported-submit-API at the "approve for live" gate (`create_candidate`, `is_file`-guarded for synthetic fixtures) — `test_create_candidate_rejects_halt_gate_bypass_strategy`.
- **iter-26 (P1):** STOP-vs-detached-restart no-pid ACK race → Phase-C re-check under the node-row lock.
- **iter-27:** Phase-C now loads the node row `with_for_update=True` (serializes vs `/stop`); on a raced stop it SIGTERMs the child but KEEPS the cached handle + leaves the row active, deferring terminalization to the reap loop (no slot reopen before the child is confirmed dead; the prior version set `status="stopped"` + dropped the handle + wrote `last_stopped_at` on the wrong model); the ownerless-active-row reaper excludes in-flight spawns via `_in_flight_spawn_rows` (a slow legitimate spawn with `pid=NULL`/no-handle-yet is no longer reaped) — `test_phase_c_kills_child_when_stop_races_after_prestart_gate` + `test_ownerless_backstop_skips_in_flight_spawn_row`.
- **iter-28 (P1):** `_mark_running` (the child's self-write of `running` once it reaches `is_running`) unconditionally promoted the node row, so an operator `/stop` that stamped `stop_requested_at`/`stopping` during the ~90s startup got overwritten → a stopped account would appear active. Fixed: skip the `running` self-promotion (and the deployment forward-sync + `record_success`) when the row carries stop intent or sits in a stop/terminal status, evaluated under the FOR UPDATE already held — `test_mark_running_skips_promotion_when_node_row_carries_stop_intent`. Full suite green, ruff + mypy --strict clean.
...
This PR replaces the guard with real binding:

- **New `services/live/snapshot_binding.py`:** `verify_member_matches_candidate` compares `member.config` (minus deploy-injected fields: `manage_stop`, `order_id_tag`, `market_exit_time_in_force`) to `candidate.config` via canonical-JSON, and `member.instruments` to `candidate.config["instruments"]` as a sorted set. Raises `BindingMismatchError` with a field-by-field diff on diverge, `BindingInstrumentsMissingError` for pre-Bug-#3 candidates that lack the stamp.
- **Pre-reserve flow in `api/live.py`:** new `_resolve_binding_for_start_portfolio` helper runs BEFORE `idem.reserve(...)` so candidate drift invalidates cached outcomes naturally. Per-member candidate resolution branches on warm-restart vs first-deploy (the warm-restart path is deterministic via `candidate.deployment_id == existing_deployment.id`; first-deploy path requires exactly one `live_candidate` row). Eligibility-stage guard rejects archived/non-eligible candidates. `binding_fingerprint` folded into the idempotency `body_hash`.
- **7 new HTTP 422 error codes:** `BINDING_EMPTY_REVISION` / `LIVE_DEPLOY_REPAIR_REQUIRED` / `BINDING_NOT_GRADUATED` / `BINDING_AMBIGUOUS` / `BINDING_INELIGIBLE` / `BINDING_MISMATCH` / `BINDING_INSTRUMENTS_MISSING`.
- **Instrument stamping at promotion:** `api/research.py` now reads `job.config["instruments"]` and writes it into the candidate's config at promotion time (both best-result and trial-index paths). Without this, the binding verifier would raise `BindingInstrumentsMissingError` on every research-graduated candidate. Manual candidates created via `POST /api/v1/graduation/candidates` must include `instruments` in their config (operator visibility).
- **Stage-link replaces orphan-query:** the PR #63 plumbing queried `stage == "live_running" AND deployment_id IS NULL` looking for an orphan to link. Now the route uses `resolved_bindings` from the pre-reserve helper, sets `candidate.deployment_id = deployment.id`, and transitions via `GraduationService.update_stage()` in the SAME DB transaction. Link failures hard-fail (no `except: log.warning`).
- **Backfill script:** `scripts/backfill_candidate_instruments.py` rewrites existing candidates whose `config` predates the instruments stamp by reading the original `research_job.config["instruments"]`. Dry-run by default; `--apply` commits. Exits non-zero if any candidate has no parent job (manual creation) so the operator can re-graduate explicitly. Operator runs once before deploying the first portfolio under the new binding flow.
- **E2E use case file:** `tests/e2e/use-cases/live/start-portfolio-snapshot-binding.md` — UC3 (divergent config → 422 BINDING_MISMATCH), UC4 (matching config + paper deploys), UC5 (real-money no-longer-blocked, operator-confirmed), UC6 (idempotency replay returns cached outcome — binding_fingerprint stable through stage transition), UC7 (candidate re-graduation invalidates cache).

**Council Option D:** this is the third + final PR of the 2026-05-13 trio (Bug #1 in #64, Bug #2 in #65, Bug #3 here). The 503 guard from PR #63 is gone — `paper_trading=false` deploys flow through binding verification.

**Verification (pre-drill):** 1933 backend unit tests pass; ruff + mypy --strict clean (188 files). Plan-review converged after 10 Codex iterations on the original plan (carried unchanged into this PR). Codex code-review round 1 (xhigh) raised 2 P1 + 1 P2 — all addressed:

- **P1:** Link + `update_stage()` now runs BEFORE `bus.publish_start` so a published START always corresponds to a linked candidate; on link failure the START command is never on Redis and the supervisor's startup-hard-timeout watchdog sweeps the row to `failed`.
- **P1:** Per-member candidate row re-queried with `SELECT FOR UPDATE` inside the deploy transaction, then `verify_member_matches_candidate` re-runs against the locked row. Catches drift (archive, edit, concurrent first-deploy) between pre-reserve resolution and link.
- **P2:** Instruments canonicalized via `lookup_for_live(as_of_date=exchange_local_today(), session=db)` on BOTH member and candidate sides — futures-roll / alias-drift safe. `verify_member_matches_candidate` gains optional `*_instruments_canonical` kwargs; the route handler passes them; the fingerprint uses the SAME canonical form so it's stable across roll boundaries. New error code `BINDING_INSTRUMENT_RESOLVE_FAILED` (HTTP 422) for resolver failures.

**Drill pending:** UC3 (binding mismatch → 422 BINDING_MISMATCH with field diff) and UC4 (matching config + paper_trading=false deploys successfully through binding, no 503 anymore). Memory entry `feedback_e2e_before_pr_for_live_fixes` requires a paper IB drill against the worktree's compose stack before merge. Operator step — Claude cannot drive IB Gateway autonomously.

### 2026-05-13 — Stop+flatness verification + TIF=DAY for US equities (`fix/stop-flatness-verification`) — **drill pending**

Bug #2 of three discovered during the 2026-05-13 paper-money drill. `POST /api/v1/live/stop` returned 200 immediately after SIGTERM, without confirming the broker actually closed positions. Nautilus's `Strategy.stop()` → `market_exit()` loop can hit `max_attempts` when IB rejects exit orders (e.g. TIF preset mismatch), leaving residual positions while the API claims success. IB stock-account default TIF preset is DAY, but Nautilus emits GTC unless overridden — IB rejects with error 10349 + cancels in the same message, while also CONFIRMING the order accepted (phantom-fill race observed during the drill).

- **Deployment-scoped flatness report:** new `LiveCommandType.STOP_AND_REPORT_FLATNESS` carries `(stop_nonce, member_strategy_id_fulls)`. Supervisor RPUSHes a ticket onto `flatness_pending:{deployment_id}` (TTL 120s + LTRIM -32 -1), then SIGTERMs the child. Child shutdown-finally hook reads `kernel.cache.positions_open()` filtered by the member set and writes `stop_report:{stop_nonce}` Redis key with the report. API polls the per-nonce key with exponential backoff (50ms→1.6s, capped) up to 30s (/stop) or 15s (/kill-all in parallel).
- **TIF=DAY for US-equity venues:** new helper `_strategy_us_equity_tif_overrides` injects `market_exit_time_in_force=int(TimeInForce.DAY)` for strategies targeting `NASDAQ`/`NYSE`/`ARCA`/`BATS`/`IEX`/`AMEX`/`XNAS`/`XNYS`. Non-US-equity (futures, FX) keeps the GTC default. Injection lands in BOTH `build_live_trading_node_config` (single-strategy) and `build_portfolio_trading_node_config` (multi-strategy) so a strategy moved between them retains its TIF override.
- **SET-NX coalescing:** atomic `SET inflight_stop:{deployment_id} <nonce> NX EX 60` ensures concurrent `/stop` callers converge on the originator's nonce. Coalesced readers share the per-nonce `stop_report` key — API never DELs the key, relies on the 120s TTL.
- **5 operational metrics:** `msai_flatness_requests_total`, `msai_flatness_coalesced_total`, `msai_flatness_poll_timeout_total`, `msai_flatness_report_non_flat_total` (counters), and `msai_flatness_pending_list_length` (gauge labeled by `deployment_id`). Alert rules suggested in the ADR.
- **ADR + runbook:** `docs/decisions/redis-flatness-protocol.md` documents `inflight_stop`/`flatness_pending`/`stop_report` keyspace, TTLs, coalescing semantics, and operator runbook (broker_flat=false / unknown / list-growing scenarios).

**Verification (pre-drill):** 1899 backend unit tests pass; ruff + mypy --strict clean (187 files). Plan-review converged after 10 Codex iterations (P1 trajectory 4→4→3→5→3→1→0→0→0→0). 5-advisor council + Codex chairman (Option D) recommended PR split — Bug #1 shipped as PR #64; Bug #2 (this branch) and Bug #3 (snapshot binding, separate PR) each get their own paper IB drill.

**Drill pending:** memory entry `feedback_e2e_before_pr_for_live_fixes` requires a paper IB drill against the worktree's compose stack before merge. Operator step — Claude cannot drive IB Gateway autonomously. After drill PASSES (broker_flat=true + IB portal confirmation), verify-e2e report lands at `tests/e2e/reports/2026-05-13-bug-2-stop-flatness.md`, branch ships.
