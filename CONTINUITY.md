...
- [x] **REAL-MONEY DRILL on U4705114 — PASSED 2026-04-20 14:26–14:27 UTC.** Report at `docs/runbooks/drill-reports/2026-04-20-live-path-registry-drill.md`. 1 share AAPL BUY @ $274.12 → `/kill-all` → SELL @ $274.02 (50s cycle). Net cost $2.11 (< $5 target). Structured log `live_instrument_resolved source=registry canonical_id=AAPL.NASDAQ` captured — registry-backed path exercised (not `canonical_instrument_id` helper). All 5 council constraints (#1 / #3 / #4 / #5 / #6) validated in docker logs. Symbol substituted AAPL-for-QQQ per runbook deviation note (QQQ outside CLI closed-universe; requires follow-up PR #3b Symbol Onboarding — PR scope unchanged).
...
- [ ] PR created — ready now. All 12 gates before this one are `[x]`.
