...
- [ ] **REAL-MONEY DRILL on U4705114** — council-mandated; exercise registry-backed path with a 1-share BUY/SELL before merge. **OPERATOR STEP — cannot self-execute.** Runbook at `docs/runbooks/drill-live-path-registry-wiring.md`. Required before PR creation per CLAUDE.md workflow.
...
- [ ] PR created — **blocked on real-money drill**. PR creation is the next step AFTER the operator runs the drill + attaches the report.
