"""Unit tests for ``ProcessManager.spawn``'s account-halt re-check.
...
the testcontainers used by ``tests/integration/test_process_manager.py``)
because the contract here is:

1. ``ProcessManager._account_id_for`` reads ``LiveDeployment.account_id``
...
from msai.live_supervisor.process_manager import ProcessManager, _PhaseAOutcome
...
def _make_pm(fake_redis: MagicMock, *, account_id: str | None = _ACCOUNT_ID) -> ProcessManager:
    """Build a ProcessManager whose phase A always succeeds and whose
